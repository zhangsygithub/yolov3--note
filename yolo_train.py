...
from utils.dataReader import Reader
from model.yolo3_model import yolo
from collections import defaultdict
from yolo_predict import yolo_predictor
from utils.utils import draw_box, load_weights, letterbox_image, voc_ap
...
def train():
   ...
   # #测试用-----------------------------------------------------------------------------------------------------------
    ...
    with tf.control_dependencies(update_ops):
        if config.pre_train:
            # 整个网络结构包括'darknet53'（主干网络）和'yolo'（3个预测分支）两个命令空间，如果是使用预训练模型，则训练的变量只训练‘yolo’命令空间下，‘darknet53’则冻结
            #train_var用于收集 'yolo'结构中可训练的变量的list
    ...
    with tf.Session(config = tf.ConfigProto(log_device_placement = False)) as sess:
        ...
        if config.pre_train is True:
            load_ops = load_weights(tf.global_variables(scope = 'darknet53'), config.darknet53_weights_path)                  #tf.global_variables(scope = 'darknet53')获得darknet中的所有变量list，用于加载模型权重
            sess.run(load_ops)
