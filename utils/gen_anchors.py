...
def convert_coco_bbox(size, box):
    ...
    y = y * dh
    h = h * dh
    return x, y, w, h                            #输出的x,y,w,h都是相对于原始图像的尺寸做了归一化的
...
def box_iou(boxes, clusters):
    ...
    ----------
        ...
        clusters: 聚类中心
    """
    box_num = boxes.shape[0]                            # n
    cluster_num = clusters.shape[0]                     #9
    box_area = boxes[:, 0] * boxes[:, 1]                #n
    #每个box的面积重复9次，对应9个聚类中心
    box_area = box_area.repeat(cluster_num)                     #[s1 s1 s1 s1 s1 s1 s1 s1 s1, s2 s2 s2 s2 s2 s2 s2 s2 s2,...]
    box_area = np.reshape(box_area, [box_num, cluster_num])     #[n,9]

    cluster_area = clusters[:, 0] * clusters[:, 1]
    cluster_area = np.tile(cluster_area, [1, box_num])           #[1,9*n]
    cluster_area = np.reshape(cluster_area, [box_num, cluster_num]) #[n,9],每一行9中anchor的面积
    ...
    iou = np.multiply(min_high, min_width) / (box_area + cluster_area - np.multiply(min_high, min_width))             #输出[n,9]的IOU
    return iou
...
def Kmeans(boxes, cluster_num, iteration_cutoff = 25, function = np.median):
    ...
    -------
        clusters: 聚类中心box的大小
    """
    boxes_num = boxes.shape[0]                    #boxes的shape为【n,2】
    best_average_iou = 0
    best_avg_iou_iteration = 0
    best_clusters = []
    anchors = []
    np.random.seed()
    # 随机选择所有boxes中的box作为聚类中心  选择9个聚类中心  [9,2]
    clusters = boxes[np.random.choice(boxes_num, cluster_num, replace = False)]
    count = 0
    while True:
        distances = 1. - box_iou(boxes, clusters)              #1-iou作为距离，shape为【n,9】
        boxes_iou = np.min(distances, axis=1)                  #每个框离最近的聚类中心的距离
        # 获取每个box距离哪个聚类中心最近
        current_box_cluster = np.argmin(distances, axis=1)
        average_iou = np.mean(1. - boxes_iou)                 #iou的平均值作为聚类效果的好坏的标准
...
def load_cocoDataset(annfile):
    ...
    for image_id in image_ids:
        ...
        for ann in anns:
            ...
            data.append(bb[2:])                                           #因为要聚类anchor的形状，因此只需要w,h即可
    return np.array(data)                                                #返回归一化的w,h ，shape为【n,2】
...
def process(dataFile, cluster_num, iteration_cutoff = 25, function = np.median):
    ...
    last_best_iou = 0
    last_anchors = []
    boxes = load_cocoDataset(dataFile)           #boxes之中只有归一化的w h,其shape为【n,2】
    box_w = boxes[:1000, 0]
    box_h = boxes[:1000, 1]
