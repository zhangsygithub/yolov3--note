...
def load_weights(var_list, weights_file):
    ...
    with open(weights_file, "rb") as fp:
        _ = np.fromfile(fp, dtype=np.int32, count=5)            #去掉前5个数
...
def letterbox_image(image, size):
    ...
    resized_image = image.resize((new_w,new_h), Image.BICUBIC)     #使用三次差值

    boxed_image = Image.new('RGB', size, (128, 128, 128))
    boxed_image.paste(resized_image, ((w-new_w)//2,(h-new_h)//2))   #将缩放后的彩色图，贴到416*416填充图中
    return boxed_image
